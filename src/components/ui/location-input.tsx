...
const LocationSelector = ({
  ...
  const defaultCountry = countriesData.find(c => c.name === defaultCountryName) || null;
  const defaultState = statesData.find(
    s => s.name === defaultStateName && s.country_id === defaultCountry?.id
  ) || null;

  const [selectedCountry, setSelectedCountry] = useState<CountryProps | null>(
    defaultCountry
  )
  const [selectedState, setSelectedState] = useState<StateProps | null>(defaultState)
  ...
  useEffect(() => {
    ...
    // Also call the callbacks so the parent knows the form is initialized
    onCountryChange?.(defaultCountry);
    onStateChange?.(defaultState);
  }, [defaultCountryName, defaultStateName]);
  ...
  return (
    <div className="flex gap-4">
      ...
      {availableStates.length > 0 && showStates && (
